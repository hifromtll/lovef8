...
export default function GuidePage() {
  ...
  const loadMessages = useCallback(async (threadId: string) => {
    ...
    setMessages((data || []) as GuideMessageRow[]);
  }, []);
  ...
  useEffect(() => {
    async function init() {
      ...
      const { data: profile, error } = await supabase
        .from('profiles')
        .select('id, username, role, app_role, is_guide, guide_gender')
        .eq('id', user.id)
        .single();
  ...
  const pageTitle = isGuide ? 'Guide Inbox' : 'Talk to Guide';
          ...
          {guideThreads.length === 0 ? (
            <div className="rounded-3xl border border-neutral-200 bg-white p-8 text-center shadow-sm">
              <div className="text-xl font-black text-neutral-900">
                {isGuide ? 'No active member guide conversations right now' : 'No active guide chat'}
              </div>
            ...
            <div className="grid min-h-0 grid-cols-[320px_minmax(0,1fr)] gap-3">
              <aside className="min-h-0 overflow-hidden rounded-3xl border border-neutral-200 bg-white shadow-sm">
                <div className="grid h-full min-h-0 grid-rows-[auto_minmax(0,1fr)] p-3">
                  <div className="px-2 pb-3 pt-1 text-sm font-black uppercase tracking-wide text-neutral-500">
                    {isGuide ? `Active guide threads (${guideThreads.length})` : 'Your guide chat'}
                  </div>
                            ...
                            <div className="flex items-start justify-between gap-3">
                              <div className="min-w-0">
                                ...
                                <div className="mt-1 text-xs text-neutral-500">
                                  Seeking: {thread.seeking || 'not set'}
                                </div>
                              </div>
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <div className="text-lg font-black text-neutral-900">
                          {isGuide ? getMemberDisplayName(activeMemberLabel) : 'Guide conversation'}
                        </div>
                        <div className="mt-1 text-sm text-neutral-600">
                          {isGuide
                            ? `Seeking: ${activeThread?.seeking || 'not set'}`
                  ...
                  <div className="relative min-h-0 overflow-y-auto px-5 py-4">
                    ...
                    {messages.length === 0 ? (
                      <div className="relative rounded-2xl border border-dashed border-neutral-200 bg-neutral-50 p-6 text-center">
                        <div className="text-base font-bold text-neutral-900">No messages yet</div>
                        <div className="mt-1 text-sm text-neutral-600">
                          {isGuide
                  ...
                  <div className="border-t border-neutral-200 px-4 py-3">
                    <div className="flex items-end gap-3">
                      <textarea
                        ...
                        placeholder={isGuide ? 'Reply as a guide…' : 'Type your question…'}
                        rows={2}
